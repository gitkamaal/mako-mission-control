...
const priorityColors: Record<string, string> = {
  ...
  medium: "bg-blue-500/30 text-blue-300",
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gradient">Task Board</h2>
          <p className="text-muted-foreground">Manage your mission tasks</p>
        </div>
        <button 
          onClick={() => setCreateOpen(true)}
          className="px-4 py-2 rounded-xl glass glass-hover text-sm font-medium glow-blue"
        >
          + New Task
        </button>
      </div>
      ...
      <div className="flex-1 flex gap-4 overflow-x-auto pb-4">
        {columns.map((column) => (
          <div 
            key={column.id} 
            className="flex-1 min-w-[300px] max-w-[350px] glass-card p-4 flex flex-col"
          >
            {/* Column Header */}
...
function TaskCard({ task }: { task: any }) {
  ...
  return (
    <div className="p-4 rounded-xl glass glass-hover cursor-pointer transition-all duration-200 hover:glow-blue">
      {/* Title & Priority */}
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-2">
          {task.assignedTo ? (
            <>
              <div className="w-5 h-5 rounded-full glass flex items-center justify-center text-[10px]">
